use bevy::math::{IVec2, UVec2, Vec2, ivec2};
...
impl Iterator for GridRectIter {
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        ...
        let count = self
            .tail
            .as_index(self.size.as_uvec2())
            .saturating_sub(self.head.as_index(self.size.as_uvec2()))
            + 1;
        (count, Some(count))
    }
}
