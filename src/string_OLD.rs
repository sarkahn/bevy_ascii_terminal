...
//     pub fn new(string: &'a str, rect: GridRect, point: PivotedPoint) -> Self {
//         let pivot = point.pivot().unwrap_or(Pivot::TopLeft);
//         let mut xy = point.calc_from_size(rect.size);
...
impl<'a> StringIter<'a> {
    pub fn new(
        ...
        wrapped: bool,
        rect: GridRect,
        point: impl Into<PivotedPoint>,
    ) -> Self {
        let point: PivotedPoint = point.into().with_default_pivot(Pivot::TopLeft);
        let pivot = point.pivot().unwrap();
        let xy = point.calc_from_size(rect.size);

        // TODO: Account for vertical line offset
        // TODO: What about centered strings?

        let remaining_width = rect.width().sub(1) - xy.x as usize;

        let (next_line, remaining) = if wrapped {
            wrap_string(string, remaining_width)
        } else {
            split_string(string, remaining_width)
        }
        .unwrap_or_default();
        Self {
            remaining,
...
#[cfg(test)]
mod tests {
    use crate::{GridPoint, GridRect, Pivot};

    use super::{wrap_string, StringIter};

    #[test]
    fn string_iter() {
        let rect = GridRect::new([0, 0], [8, 3]);
        let string = "Hello";
        let iter = StringIter::new(string, false, rect, [5, 0]);
    ...
    fn iter() {
        ...
        //let iter = WrappedStringIter::new(string, rect, [0, 0].pivot(Pivot::TopLeft));
        ...
        // let iter = WrappedStringIter::new(string, rect, [0, 0].pivot(Pivot::TopRight));
        ...
        // let iter = WrappedStringIter::new(string, rect, [0, 0].pivot(Pivot::BottomRight));
