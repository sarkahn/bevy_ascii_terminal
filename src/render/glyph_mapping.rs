...
impl GlyphMapping {
    pub fn new(mapping: HashMap<char,usize>, width: usize, height: usize) -> Self {
    ...
    fn from_file(path: &str, width: usize, height: usize) -> Self {
        let f = File::open(path).expect("Error opening file");
        let mapping = from_reader(f).expect("Error parsing mapping file");
        GlyphMapping::new(mapping, width, height)
    }

    pub fn code_page_437() -> Self {
        GlyphMapping::from_file("assets/code_page_437.mapping", 16, 16)
    }

    pub fn get_glyph(&self, index_x: usize, index_y: usize) -> char {
        *self.reverse_mapping.get(&(index_x,index_y)).unwrap_or(&' ')
    }
