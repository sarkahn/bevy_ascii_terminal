...
fn setup( 
    ...
    term.put_char([0,0].pivot(Pivot::Center), 'O'.fg(Color::WHITE).bg(Color::BLACK));
...
fn update(
    ...
    if let Some(window) = windows.get_primary() {
        for (mut term, tw) in q_term.iter_mut() {
            term.row_iter_mut(0).for_each(|t| *t = Tile::default() );
            term.row_iter_mut(1).for_each(|t| *t = Tile::default() );
